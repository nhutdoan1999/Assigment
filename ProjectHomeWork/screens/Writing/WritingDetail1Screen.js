...
const WritingDetail1Screen = () => {
    ...
    const submitAnswers = async () => {
        try {
            const scoringResponse = await fetch('http://10.60.1.203:8080/data/openai/score', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    responseType: 'json',
                    testID: item.wri_id,
                    answer_1: answers.answer1,
                    answer_2: answers.answer2
                }),
            });
            ...
            if (scoringResponse.ok) {
                ...
                navigation.navigate('ResultWritingScreen', { data: scoreData.data });
            } else {
                const errorText = await scoringResponse.text();
                console.log('Scoring Error:', errorText);
                Alert.alert('Error', 'There was a problem getting the score.');
            }
        } catch (error) {
...
export default WritingDetail1Screen;
